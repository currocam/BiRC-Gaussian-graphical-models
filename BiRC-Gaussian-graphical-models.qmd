...
![**Inferring a microbial network involves transforming the data and determining the graph structure from a sparse precision matrix estimate.** \newline (a) Abundances of five different taxa across 200 samples, simulated according to a Poisson process with unequal depth. (b) Centered log-ratio abundances to address compositionality and the assumption of normality. (c) and (d) corresponds to the graphical lasso estimation for $\lambda = 0.26$. (c) is the undirected graph determined from the zero-pattern of the precision matrix in (d). False positive edges are shown in light red, and false negatives in grey.](figures/02-conceptual_figure.pdf){#fig-conceptual}
...
It is possible to infer the graph by first estimating the inverse of the covariance matrix, known as the precision matrix, $\Omega := \Sigma^{-1}$. The precision matrix is useful because, for every matrix element, it is true that $\Omega_{ij}=0$ if and only if the $i$ and $j$ are conditionally independent given the rest of the dimensions. Then, we can unambiguously determine the graph structure from the pattern of zero entries in the precision matrix (@fig-conceptual c and @fig-conceptual d). Formally, we say that a random variable $X \in \mathbb R^p$ follows $G = (V, E)$ if it is distributed as $\mathcal N_p(0, \Omega ^{-1})$, where $\Omega$ is a positive definite matrix of dimensions $p\times p$ such that $\Omega_{ij} = 0$ implies $(i, j) \notin E$.
...
Unfortunately, microbial abundance data is not normal (@fig-conceptual a). In the literature, methods overcome this issue by applying some transformation over the original discrete counts. Alternatively, methods that simultaneously estimate the precision matrix and a set of $p$ normal latent variables from the $p$ observed variables are known as Copula Gaussian graphical models.
...
However, this normalization imposes a sum-to-one constraint, making the relative abundances, $X \in \mathbb R^+_{n\times p}$, of the different taxa no longer independent. @kurtz2015 proposed to apply the centered log-ratio transformation @eq-clr to the relative abundances and estimate the Gaussian graphical model from the transformed data $X \in \mathbb R_{n\times p}$ (@fig-conceptual c).
