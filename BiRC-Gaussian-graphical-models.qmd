...
In my opinion, this decision-rule makes more difficult to estimate the uncertainty of all network metrics in a transparent way. @jongerling2023 explored method to overcome the fact that applying the decision rule when estimating the posteriors a centrality metrics requires to know the whole posterior distribution *beforehand*.
